...
.actualite-unique-scope .actualite-content button {
  margin-bottom: 10px;
  margin-top: 5px;
}
...
.pulse {
  animation: pop-effect 0.35s ease;
}
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        if ($type === 'image') {
            echo "<div class='media-wrapper' data-id='{$id}' onclick='handleImageClick(this)'>";
        ...
        echo "<p class='images-vues vue-counter' data-id='{$id}' data-count='{$vues}'><i class='fas fa-eye'></i> " . format_views($vues) . " vues</p>";
        ...
        echo "<div class='share-container'>
              ...
              echo "<button id='like-btn-{$id}' onclick='ajouterLike({$id})' class='like-btn'>
        <i class='fas fa-thumbs-up'></i> <span id='like-count-{$id}'>0</span> J’aime
      </button>";
...
function compterVue(id) {
  fetch("/Plateforme_web_du_FECACFOP/view/Actualites/increment_vue_actualite.php", {
    method: "POST",
...
// ✅ Ne compte qu'une seule vue par utilisateur pour un contenu
function comptageConditionnel(id) {
  if (!sessionStorage.getItem("vue_" + id)) {
    sessionStorage.setItem("vue_" + id, "1");
    compterVue(id);
  }
}
...
function handleModalClick(button) {
  ...
  comptageConditionnel(id); // ✅ Vue comptée une seule fois
  openModal(title, description, media, type);
}

function handlePostuler(button) {
  const id = button.dataset.id;
  comptageConditionnel(id);
  // Tu peux déclencher ici l'ouverture du formulaire
}
...
<audio id="like-sound" src="/Plateforme_web_du_FECACFOP/public/assets/sounds/like.mp3" preload="auto"></audio>
<script>
function ajouterLike(id) {
  const son = document.getElementById("like-sound");
  const btn = document.getElementById("like-btn-" + id);

  $.post("/Plateforme_web_du_FECACFOP/view/Actualites/ajouter_like.php", { article_id: id }, function(response) {
    // Si c’est un like, on joue le son et on applique le style
    if (response === "liked") {
      if (son) son.play();
      if (btn) btn.classList.add("liked");
    }

    // Si l’utilisateur annule son like, on retire le style
    if (response === "unliked" && btn) {
      btn.classList.remove("liked");
    }

    // Effet visuel à chaque clic
    if (btn) {
...
function chargerLikes(id) {
  $.get("/Plateforme_web_du_FECACFOP/view/Actualites/recuperer_likes.php", { article_id: id }, function(data) {
    document.getElementById("like-count-" + id).innerText = data;
  });
}

// Charger tous les likes au chargement
document.addEventListener("DOMContentLoaded", function() {
  <?php
    $result->data_seek(0);
    while ($row = $result->fetch_assoc()) {
        echo "chargerLikes({$row['id']});\n";
    }
  ?>
});
</script>
