...
<head>
 ...
 <style>
        ...
        box-shadow: 0 6px 20px rgba(0,0,0,0.05);
          margin-bottom: 60px; /* 🔥 Ajoute ceci */
...
.btn-candidature-unique:hover {
  background: #0056b3;
}
#modal-description {
  max-height: 400px;           /* Limite la hauteur pour éviter que tout déborde */
  overflow-y: auto;            /* Active le scroll vertical si nécessaire */
  padding: 12px;
  line-height: 1.7;
  text-align: justify;
  white-space: pre-wrap;       /* Garde les sauts de ligne si description le contient */
  border: 1px solid #eee;
  background: #f9f9f9;
  border-radius: 6px;
}
...
<div class="background">
    <h1 id="animated-title-emploi" class="animated-title-emploi"></h1>
</div>
...
echo "<p class='emploi-vues vue-counter' data-id='{$id}' data-count='{$vues}'><i class='fas fa-eye'></i> " . format_views($vues) . " vues</p>";
echo "<p class='actualite-description'>" . substr($description, 0, 100) . "...</p>";

// ✅ LIRE PLUS (modifié avec data-id)
echo "<button class='btn-details-view'
  data-id='{$id}'
  data-title=\"{$titre}\"
  data-description=\"" . htmlspecialchars($row['description'], ENT_QUOTES) . "\"
  data-media=\"/Plateforme_web_du_FECACFOP/uploads/emplois/" . basename($media) . "\"
  onclick='handleModalClick(this)'>Lire plus</button>";


// ✅ POSTULER (modifié avec data-id)
echo "<button class='btn-candidature-unique'
  data-id='{$id}'
  onclick='handlePostuler(this)'>Postuler maintenant</button>";
