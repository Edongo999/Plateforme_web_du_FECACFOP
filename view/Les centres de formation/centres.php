<?php
$sql = "SELECT * FROM centres_inscrits";
$result = $conn->query($sql);

while ($row = $result->fetch_assoc()):
?>
    <div class="col-lg-4 col-md-6 portfolio-item <?php echo strtolower($row['localisation']); ?> wow fadeInUp">
        <div class="rounded overflow-hidden">
            <div class="position-relative overflow-hidden">
                <img class="img-fluid w-100" src="<?php echo htmlspecialchars($row['logo_path']); ?>" alt="">
                <div class="portfolio-overlay">
                    <a class="btn btn-square btn-outline-light mx-1" href="<?php echo htmlspecialchars($row['logo_path']); ?>" data-lightbox="portfolio"><i class="fa fa-eye"></i></a>
                </div>
            </div>
            <div class="border border-5 border-light border-top-0 p-4">
                <p class="text-primary fw-medium mb-2"><?php echo htmlspecialchars($row['localisation']); ?></p>
                <h5 class="lh-base mb-0"><?php echo htmlspecialchars($row['nom_centre']); ?></h5>
            </div>
        </div>
    </div>
<?php endwhile; ?>
        ...
        <div class="centre-card-container">
            <?php while ($row = $result->fetch_assoc()): ?>
                <div class="centre-card shadow-sm">
                    <img src="http://localhost/Plateforme_web_du_FECACFOP/view/les centres de formation/uploads/<?php echo basename($row['logo']); ?>" class="centre-card-img-top" alt="<?php echo $row['nom']; ?>">
